...
// ospwgen <format> [count] [h|H|h0|H0|j]
...
void help(const char *cmd) {
    ...
    printf("Old School Password Generator - Revision %04x\n", REV);
...
int parse_positive_int(const char *s, int *out) {
    ...
    v = strtol(s, &end, 10);
    if (*end != '\0') return 0;
    if (v <= 0 || v >= MAX_PASSWORD_LENGTH + 1) return 0;
...
void validate_format(const char *fmt, const char *cmd) {
    ...
    if (len > MAX_PASSWORD_LENGTH + 1) {
        printf("ERROR: Format string must be %d bytes or LESS!\n", MAX_PASSWORD_LENGTH);
        usage(cmd);
    }
...
void generate_random_password(char *out, int length) {
    ...
    for (int i = 0; i < length; i++) {
        out[i] = a_all[arc4random_uniform(l_all)];
    }
    out[length] = '\0';
}
...
void generate_from_format(const char *fmt, char *out) {
    ...
    for (size_t i = 0; i < len; i++) {
        switch (fmt[i]) {
        case 'u': out[i] = a_upper [arc4random_uniform(sizeof(a_upper)  - 1)]; break;
        case 'l': out[i] = a_lower [arc4random_uniform(sizeof(a_lower)  - 1)]; break;
        case 'c': out[i] = a_lowerc[arc4random_uniform(sizeof(a_lowerc) - 1)]; break;
        case 'v': out[i] = a_lowerv[arc4random_uniform(sizeof(a_lowerv) - 1)]; break;
        case 'C': out[i] = a_upperc[arc4random_uniform(sizeof(a_upperc) - 1)]; break;
        case 'V': out[i] = a_upperv[arc4random_uniform(sizeof(a_upperv) - 1)]; break;
        case 'd': out[i] = a_digit [arc4random_uniform(sizeof(a_digit)  - 1)]; break;
        case 's': out[i] = a_symbl [arc4random_uniform(sizeof(a_symbl)  - 1)]; break;
        case 'r': out[i] = a_all   [arc4random_uniform(sizeof(a_all)    - 1)]; break;
        }
    }
    out[len] = '\0';
}
...
void print_json_array(const char passwords[][MAX_PASSWORD_LENGTH + 1], int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        printf("    \"password\": \"%s\",\n", escaped);
        printf("    \"hex\": \"");
        for (size_t k = 0; k < strlen(passwords[i]); k++) {
...
void handle_random_mode(int argc, char **argv, char *out, const char *cmd) {
    int length = DEFAULT_PASSWORD_LENGTH + 1;
    int count  = 1;
    int tmp;
